...
def run_one(filepath):
    ...
    if entry and "zones_m2" in entry:
        ...
        for zone in payload.get("zones", []) or []:
            category = zone.get("category") if isinstance(zone, dict) else None
            zone_area = zone.get("area_m2") if isinstance(zone, dict) else None
            if category and isinstance(zone_area, (int, float)) and not isinstance(zone_area, bool):
                actual_zones[category] = actual_zones.get(category, 0.0) + float(zone_area)

        zone_tol_pct = entry.get("zone_tol_pct", entry.get("tol_pct", 2))
        zone_details = {}
        all_zones_pass = True
        for category, gold_zone_area in entry["zones_m2"].items():
            actual_zone_area = actual_zones.get(category)
            if actual_zone_area is None:
                delta_pct = None
                verdict = "GOLD_FAIL"
                all_zones_pass = False
            elif gold_zone_area:
                delta_pct = abs(actual_zone_area - gold_zone_area) / gold_zone_area * 100
                verdict = "GOLD_PASS" if delta_pct <= zone_tol_pct else "GOLD_FAIL"
                all_zones_pass = all_zones_pass and verdict == "GOLD_PASS"
            else:
                delta_pct = 0.0 if actual_zone_area == 0 else None
                verdict = "GOLD_PASS" if actual_zone_area == 0 else "GOLD_FAIL"
                all_zones_pass = all_zones_pass and verdict == "GOLD_PASS"
            zone_details[category] = {
                "area_m2": round(actual_zone_area, 3) if actual_zone_area is not None else None,
                "gold_area_m2": gold_zone_area,
                "delta_pct": round(delta_pct, 2) if delta_pct is not None else None,
                "verdict": verdict,
            }
        result["gold_zone_details"] = zone_details
        result["gold_zone_verdict"] = "GOLD_PASS" if all_zones_pass else "GOLD_FAIL"
...
def main():
    ...
    zone_gold_pass = sum(1 for r in results if r.get("gold_zone_verdict") == "GOLD_PASS")
    zone_gold_fail = sum(1 for r in results if r.get("gold_zone_verdict") == "GOLD_FAIL")
    ...
    if zone_gold_pass or zone_gold_fail:
        summary_line += f" | zone gold: {zone_gold_pass} PASS, {zone_gold_fail} FAIL"
    ...
    lines.append("| File | Class | Exit | Elapsed(s) | Area(m2) | Gold(m2) | Delta% | Gold Verdict | Zone Gold | Notes / Flags |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|")
    for r in results:
        ...
        zone_details = "; ".join(
            f"{category}: {detail['area_m2'] if detail['area_m2'] is not None else 'missing'}"
            f"/{detail['gold_area_m2']}m2, "
            f"delta={detail['delta_pct'] if detail['delta_pct'] is not None else 'n/a'}%, "
            f"{detail['verdict']}"
            for category, detail in r.get("gold_zone_details", {}).items()
        )
        zone_report = ((r.get("gold_zone_verdict") or "")
                       + (f" ({zone_details})" if zone_details else ""))
        lines.append(
